...
void acakUrutanDokter() {
    // Mengacak urutan linked list dokter menggunakan Fisher-Yates shuffle
    ...
    // Rebuild linked list dari array yang sudah diacak
    head = arr[0];
    tail = arr[count-1];
...
void jadwalotomatis30hari(char jadwal[30][3][6][100]) {
    ...
    // Array untuk melacak dokter yang sudah bekerja per shift per hari
    char dokter_shift_hari_ini[3][100][100]; // [shift][dokter][nama]
    int count_dokter_per_shift[3] = {0};

    for (int hari = 0; hari < 7; hari++) {
        //resetShiftMingguan();
        acakUrutanDokter();
        
        // Reset tracker untuk hari baru
        for (int s = 0; s < 3; s++) {
            count_dokter_per_shift[s] = 0;
        }
        ...
        for (int s = 0; s < 3; s++) {
            ...
            while (curr && count[s] < max_dokter[s]) {
                ...
                int sudah_bekerja_shift_ini = 0;
                for (int i = 0; i < count_dokter_per_shift[s]; i++) {
                    if (strcmp(dokter_shift_hari_ini[s][i], curr->nama) == 0) {
                        sudah_bekerja_shift_ini = 1;
                        break;
                    }
                }
                
                if (!sudah_bekerja_shift_ini && curr->shift > 0 && cekPreferensi(curr->preferensi, shiftLabel[s])) {
                    strcpy(pola_mingguan[hari][s][count[s]++], curr->nama);
                    strcpy(dokter_shift_hari_ini[s][count_dokter_per_shift[s]++], curr->nama);
                    curr->shift--;
                }
                curr = curr->next;
            }
        ...
        // Isi sisa slot dengan dokter yang belum bekerja di shift ini
        for (int s = 0; s < 3; s++) {
            struct datadokter *curr = head;
            while (curr && count[s] < max_dokter[s]) {
                int sudah_bekerja_shift_ini = 0;
                ...
                if (!sudah_bekerja_shift_ini && curr->shift > 0) {
                    strcpy(pola_mingguan[hari][s][count[s]++], curr->nama);
                    strcpy(dokter_shift_hari_ini[s][count_dokter_per_shift[s]++], curr->nama);
                    curr->shift--;
                }
